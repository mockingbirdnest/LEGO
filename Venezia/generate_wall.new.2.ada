...
procedure Generate_Wall is
    ...
    Left        : constant Options.Constraint         := Options.Left;
    ...
    Right       : constant Options.Constraint         := Options.Right;
    ...
    function Compute_Fitness (Genome : Genetics.Genome;
        ...
        subtype Real_Row is Positive range 1 .. Options.Height;
        ...
        use type Genetics.Gene;
    begin
        Row        := Boolean'Pos (Bottom = null);
        Part_Width := 1;
        Column     := 1; --Natural (Genome'First mod Width_Minus_One);
        for I in Genome'Range loop
            At_Edge_Of_Aligned_Rows :=
               Row in Real_Row'First + 1 .. Real_Row'Last and then
                  ((Left /= null and then
            ...
            if Column = Positive (Width_Minus_One) then
                ...
                if Genome (I) then
                    Result := Result + Part_Cost (Part_Width) +
                                 Part_Cost (1);
                    if Row in Real_Row and then
                       not At_Edge_Of_Aligned_Rows and then
                       Genome (I - Width_Minus_One) then
                        Result := Result + Separation_Cost;
                    end if;
                else
                    if Right = null or else (Row in Real_Row and then
                                             Column < Width - Right (Row)) then
                ...
                if Genome (I) then
                    if Left = null or else (Row in Real_Row and then
                                            Column > Left (Row)) then
                        Result := Result + Part_Cost (Part_Width);
                    end if;
                    Part_Width := 0;
                    if Row in Real_Row and then
...
begin
    ...
    Anfr.Reset (Color_Generator, Options.Height * Options.Width);
    ...
    if Left /= null then
        Ada.Text_Io.Put_Line (File, "0 /left " & Options.Image (Left));
    end if;
    ...
    if Right /= null then
        Ada.Text_Io.Put_Line (File, "0 /right " & Options.Image (Right));
    end if;
    ...
    declare
        ...
        procedure Output_Part (First_Stud, Last_Stud : Positive;
            ...
            No_Bottom_Row : constant Natural  := Boolean'Pos (Bottom = null);
            Part_Width    : constant Positive :=
               Last_Stud - First_Stud + 1;
            C             : Lego.Color;
            Corner_Stud   : Natural           := 0;
            Is_2X2        : Boolean;
            Is_Left_Right : Boolean           := False;
            Is_Tee        : Boolean           := False;
            Is_Top_Bottom : Boolean           := False;
            P             : Lego.Part;
            Hide          : Boolean;
            Skip          : Boolean;
            ...
            use type Lego.Color;
        begin
            if Left /= null and then No_Bottom_Row - S.Y in Left'Range and then
               Last_Stud <= Left (No_Bottom_Row - S.Y) then
                Is_Left_Right := True;
            end if;
            if Right /= null and then
               No_Bottom_Row - S.Y in Right'Range and then
               First_Stud >= Width - Right (No_Bottom_Row - S.Y) + 1 then
                Is_Left_Right := True;
            end if;
            ...
            if (Bottom /= null and then S.Y = 0) or else
               (Top /= null and then S.Y = 1 - Height) then

                -- The constraint line.  For historical reasons, this is the only one that doesn't consume colors.
                Is_Top_Bottom := True;
                C             := Lego.Clear;
            else

                -- Pick a color.  Make sure that the color is available for the part, and that 
                -- we don't have two consecutive parts of the same color.
                C := Random (P);
