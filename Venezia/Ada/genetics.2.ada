...
package body Genetics is
	...
	function "+" (G : Gene'Base; I : Integer) return Gene'Base is
	begin
	    return G + Gene'Base (I);
	end "+";

	function "+" (I : Integer; G : Gene'Base) return Gene'Base is
	begin
	    return Gene'Base (I) + G;
	end "+";
	...
	function "-" (G1, G2 : Gene'Base) return Integer is
	begin
	    return Integer (G1) - Integer (G2);
	end "-";
	...
	function Pick (F : Access_Constant_Fitnesses) return Individual is
	    -- Use extra precision here to preserve compatibility with Windows.
	    type Fitness is new Long_Long_Float;
	    Key : constant Fitness :=
	       Fitness (Float_Random.Random (Float_Generator)) *
		  Fitness (F.Cumulative (Individual'Last));
	...
	begin
		...
		M := (U + L) / 2;
		if Key < Fitness (F.Cumulative (M)) then
		    U := M - 1;
		elsif Key > Fitness (F.Cumulative (M)) then
		    L := M + 1;
    ...
    procedure Process_Population
		 (P : Access_Constant_Population; S : Access_All_State) is
    ...
    function Compute_Fitnesses (P : Access_Constant_Population; O : Options)
			       return Fitnesses is
	...
	procedure Compute_All_Individual_Fitnesses is
	   new Process_Population (State                 => Fitnesses,
				   Access_All_State      =>
				      Access_All_Fitnesses,
				   Process_Subpopulation =>
	...
	Compute_All_Individual_Fitnesses (P => P, S => Result'Unchecked_Access);
